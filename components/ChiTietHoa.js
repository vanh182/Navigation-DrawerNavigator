...
function ChiTietHoa({route, navigation}){
    const{ tenhoa} =route.params;
    const{tenloai} =route.params;
    const{mahoa} =route.params;
    ...
    const hoachon=hoa.filter(Chitiet);

    function Chitiet(hoa) {
        return hoa.tenhoa == tenhoa;
       }

    return (
        <View style={{flex:1, alignItems:'center', justifyContent: 'center'}}>
            <Text style={styles.texttitle}>Loại Hoa :{tenloai}</Text>
        ...
        renderItem={({item})=>(
            <View>
                ...
                <Text style={styles.text}>Mã Hoa :{mahoa}</Text>
                <Text style={styles.text}>Tên Hoa :{tenhoa}</Text>
             <Text style={styles.text}>{item.mota}</Text>
                <Text style={styles.text}>Giá Bán: {item.dongia}</Text>
